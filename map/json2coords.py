...
def outscheme(a, b):
    print str(a) + ':' + str(vhNums.index(b))
...
def pathandtype(end, s):
    ...
    if new_change:
        if new_chain[0][1] * k <= (new_change[1][1] - k) * k:
            ...
            s = 'chain'
            l = new_chain[0][1]
            if new_chain[1][0] != 'end' and args.top == '1':
                outscheme(new_chain[0][1], new_chain[0][0])
                outscheme(new_chain[1][1], new_chain[1][0])
#                print str(new_chain[0][1]) + ':' + str(vhNums.index(new_chain[0][0]))
#                print str(new_chain[1][1]) + ':' + str(vhNums.index(new_chain[1][0]))
            elif args.top == '1':
#            print new_chain[0][0], new_chain[0][1], '---', end3scaf[0], end3scaf[1]
#                print str(new_chain[0][1]) + ':' + str(vhNums.index(new_chain[0][0]))
                outscheme(new_chain[0][1], new_chain[0][0])
        else:
            new_end = new_change[1]
...
#        pdb.write(template.format(t) + '\n')
        if args.top == '2':
            print str(t[3]) + ':' +  str(t[5]) + ':' + str(t[8]) + ':' + str(vhNums.index(t[7]))
#    else:
#        pdb.write(atom[0])
...
#    print tname, length
    if tname:
        t = ['ATOM'] + [a_sc] + [aname] + [tname] + [nlname] + [a_sc] + [ox, oy] + [z] + [1.00, 0.00] + [aname]
        outpdb.append(t)
...
#if endtmp:
if args.top == 1:
    print str(end5scaf[1]) + '/' + str(vhNums.index(end5scaf[0]))
...
#        vh = part[1]
#        beg = part[2][0]
#        print part[1], part[2][0], '---', part[1], part[2][1]
